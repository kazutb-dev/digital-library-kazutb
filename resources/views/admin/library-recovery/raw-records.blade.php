...
@section('content')
    ...
    <form method="GET" action="{{ $links['raw_records'] }}" class="admin-card mb-7 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6">
        ...
        <div><label class="admin-label" for="raw-map">{{ __('library_recovery.raw.mapping_status') }}</label><select id="raw-map" class="admin-input" name="mapping_status"><option value="">{{ __('library_recovery.all') }}</option>@foreach($mappingStatuses as $status)<option value="{{ $status }}" @selected(($filters['mapping_status'] ?? '') === $status)>{{ $status }}</option>@endforeach</select></div>
        <div><label class="admin-label" for="raw-apply">{{ __('library_recovery.raw.apply_status') }}</label><select id="raw-apply" class="admin-input" name="apply_status"><option value="">{{ __('library_recovery.all') }}</option>@foreach($applyStatuses as $status)<option value="{{ $status }}" @selected(($filters['apply_status'] ?? '') === $status)>{{ $status }}</option>@endforeach</select></div>
    ...
    <section class="admin-card">
        <div class="overflow-x-auto"><table class="admin-table min-w-[980px]"><thead><tr><th>ID</th><th>{{ __('library_recovery.raw.batch_id') }}</th><th>{{ __('library_recovery.raw.source_doc_id') }}</th><th>{{ __('library_recovery.raw.control_number') }}</th><th>{{ __('library_recovery.raw.mapping_status') }}</th><th>{{ __('library_recovery.raw.apply_status') }}</th><th>{{ __('library_recovery.raw.catalogue_record') }}</th><th>{{ __('library_recovery.raw.source_hash') }}</th><th></th></tr></thead><tbody>
            @forelse ($records as $record)<tr><td>{{ $record->id }}</td><td>{{ $record->legacy_import_batch_id }}</td><td>{{ $record->source_doc_id }}</td><td>{{ $record->control_number ?: '—' }}</td><td>{{ $record->mapping_status ?: '—' }}</td><td>{{ $record->apply_status }}</td><td>{{ $record->bibliographic_record_id ?: '—' }}</td><td class="max-w-[18rem] break-all font-mono text-[11px]">{{ $record->source_hash }}</td><td><a class="font-bold text-secondary hover:underline" href="{{ $record->detail_url }}">{{ __('library_recovery.view') }}</a></td></tr>@empty<tr><td colspan="9" class="text-center text-slate-500">{{ __('library_recovery.empty') }}</td></tr>@endforelse
        </tbody></table></div>
