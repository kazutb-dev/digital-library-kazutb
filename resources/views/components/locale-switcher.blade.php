...
<style>
    ...
    .locale-switcher__option:hover,.locale-switcher__option:focus-visible{background:#edf7f6;color:#075e5e;outline:none}
    .locale-switcher__option[aria-current=true]{background:#e8f2f7;color:#0f4c81;font-weight:800}
    .locale-switcher__option-label{min-width:2.35rem;font-size:.7rem;font-weight:800;letter-spacing:.045em;color:#0f6eaa}
...
<details {{ $attributes->class(['locale-switcher', 'locale-switcher--'.$resolvedVariant]) }} data-locale-switcher data-locale-variant="{{ $resolvedVariant }}">
    ...
    >
        ...
        <span>{{ __('locale.labels.'.$activeLocale) }}</span>
    </summary>
    <div class="locale-switcher__menu locale-switcher__menu--{{ $resolvedAlign }}" role="menu" aria-label="{{ __('shell.language_switcher') }}">
        @foreach (\App\Support\LocaleResolver::SUPPORTED as $locale)
            <form method="POST" action="{{ route('locale.update') }}" role="none">
                ...
                >
                    <span><span class="locale-switcher__option-label">{{ __('locale.labels.'.$locale) }}</span> {{ __('locale.names.'.$locale) }}</span>
